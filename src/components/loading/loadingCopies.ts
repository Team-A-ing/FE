export const loadingSteps = [
  { step: 1, label: '대화를 텍스트로 바꾸는 중', icon: '🎧' },
  { step: 2, label: '행간의 의미를 읽는 중', icon: '🔍' },
  { step: 3, label: '숫자로 정리하는 중', icon: '📊' },
  { step: 4, label: '피드백을 다듬는 중', icon: '💡' },
];

export const witCopies = {
  leader: [
    '팀원이 "괜찮다"고 했을 때, 진짜 괜찮은 건지 확인 중...',
    '숨은 블로커를 워드클라우드에 올리는 중...',
    '리더십 점수를 깎지 않게 조심하는 중...',
  ],
  member: [
    '리더가 놓친 당신의 강점을 찾고 있어요...',
    '커리어 메모리에 오늘의 성장을 기록하는 중...',
    '다음 미팅에서 쓸 한마디를 고르는 중...',
  ],
};

export const ANALYSIS_STEPS = [
  { icon: "🎧", label: "대화를 텍스트로 바꾸는 중...", tag: "STT 변환" },
  { icon: "🔍", label: "행간의 의미를 읽는 중...", tag: "NLP 분석" },
  { icon: "📊", label: "3대 Gap을 계산하는 중...", tag: "스코어링" },
  { icon: "💡", label: "피드백을 다듬는 중...", tag: "피드백 생성" },
] as const;

export const WITTY_COPIES = {
  leader: [
    "\"팀원이 '괜찮다'고 했을 때, 진짜 괜찮은 건지 확인 중...\"",
    "\"말 속에 숨은 진짜 감정을 찾고 있어요...\"",
    "\"리더십의 온도를 측정하는 중...\"",
    "\"더 나은 대화를 위한 인사이트를 만들고 있어요...\"",
  ],
  member: [
    "\"당신의 목소리에 담긴 의미를 해석하는 중...\"",
    "\"솔직한 피드백을 정리하고 있어요...\"",
    "\"대화의 균형을 분석하는 중...\"",
    "\"성장을 위한 인사이트를 준비 중...\"",
  ],
} as const;